...
app = FastAPI(title="CORDDSBase Temporal Accident AI")
...
def score_window(cam):
    ...
    motion=np.array([x["motion"] for x in items],dtype=np.float32)
    flow=np.array([x["flow"] for x in items],dtype=np.float32)
    def spike(v):
        base=v[:-4] if len(v)>4 else v
        med=float(np.median(base)); mad=float(np.median(np.abs(base-med)))+1e-5
        return max(0.0,min(1.0,float((v[-1]-med)/(6.0*1.4826*mad))))
    motion_spike=spike(motion)
    flow_spike=spike(flow)
    ...
    if pairs:
        key,pair_best=max(pairs.items(),key=lambda kv:kv[1])
        pair_name=f"CAR #{key[0]} + CAR #{key[1]}"
    # A brief impact can occur in one frame; no persistent contact is required.
    confidence=min(0.99,0.48*motion_spike+0.32*flow_spike+0.20*pair_best)
    reason=f"40-frame temporal window; motion spike {motion_spike:.2f}; optical-flow spike {flow_spike:.2f}"
    if pair_name:reason+=f"; nearest pair {pair_name} contact {pair_best:.2f}"
...
def frame(f:Frame):
    ...
    if previous is not None:
        ...
        flow_value=float(np.percentile(mag,90))
    ...
    return {"accident":accident,"confidence":round(confidence,3),"reason":reason,"pair":pair,"tracks":tracks,"window_frames":len(cam),"window_seconds":round(len(cam)/8,2)}
