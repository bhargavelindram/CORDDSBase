...
const S={room:null,role:null,tokenId:DEFAULT_TOKEN,roomName:"",cameras:new Map(),markers:new Map(),alerts:[],alertCooldown:new Map(),collisions:new Map(),removedCameras:new Set(),ai:{session:null,loading:false,running:false,cameras:new Map(),timers:new Map(),ort:null},map:null,watchId:null,db:null,audioCtx:null,accidentApi:"",face:{detector:null,loading:false}};
...
async function loadFaceBlur(){
if(S.face.detector||S.face.loading)return S.face.detector;
S.face.loading=true;
try{
const vision=await import("https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision");
const fileset=await vision.FilesetResolver.forVisionTasks("https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision/wasm");
S.face.detector=await vision.FaceDetector.createFromOptions(fileset,{baseOptions:{modelAssetPath:"https://storage.googleapis.com/mediapipe-models/face_detector/blaze_face_short_range/float16/1/blaze_face_short_range.tflite",delegate:"CPU"},runningMode:"VIDEO",minDetectionConfidence:.35,minSuppressionThreshold:.3});
return S.face.detector;
}catch(e){console.error("face blur",e);return null}finally{S.face.loading=false}
}
async function startRecording(id){
const c=S.cameras.get(id);if(!c||c.recording)return;
const video=c.video;const capture=video.captureStream?.bind(video)||video.mozCaptureStream?.bind(video);
...
if(privacy){
const detector=await loadFaceBlur();
...
const render=()=>{
if(!c.recording&&c.recorder==null)return;
ctx.filter="none";ctx.drawImage(video,0,0,canvas.width,canvas.height);
if(detector&&video.readyState>=2&&video.currentTime!==lastFaceTime){
try{
lastFaceTime=video.currentTime;
const result=detector.detectForVideo(video,performance.now());
faces=(result.detections||[]).map(d=>d.boundingBox).filter(Boolean);
}catch(e){}
}
...
c.recording=true;render();
}else{
stream=capture();c.recording=true;
}
let chunks=[];let started=Date.now();
const begin=()=>{
chunks=[];started=Date.now();c.recorder=new MediaRecorder(stream,{mimeType});
c.recorder.ondataavailable=e=>{if(e.data.size)chunks.push(e.data)};
c.recorder.onstop=async()=>{
const blob=new Blob(chunks,{type:mime});if(blob.size>1000)await saveRecording({camera:c.name,started,ended:Date.now(),blob,type:mime});
if(c.recording)begin()
};
c.recorder.start(1000);c.recordTimer=setTimeout(()=>{if(c.recorder?.state==="recording")c.recorder.stop()},SEGMENT_MS)
};
begin();
